 ...
 */
public class AspectExcludeObjectRelationsFilter implements AEFilter
{

	/** The person ids. */
	private Vector<String> _personIds;

	/**
	 * Instantiates a new aspect exclude object relations filter.
	 * @param personIds the person ids
	 */
	public AspectExcludeObjectRelationsFilter(final Vector<String> personIds)
	{
		this.setPersonIds(personIds);
	}
	 ...
	 */
	public final Vector<String> getPersonIds()
	{
		return _personIds;
	}
			...
			if (element instanceof Aspect)
			{
				...
				if (a.getRelationDim() != null && !a.getRelationDim().getRelationStms().isEmpty())
				{
					for (int j = 0; j < a.getRelationDim().getRelationStms().size(); j++)
					{
						...
						{
							for (String s : _personIds)
							{
								if (s.equals(relationStm.getSubject().toString()))
								{
									return true;
								}
							}
							...
							if (a.getPdrId().equals(relationStm.getSubject()) && relationStm.getRelations() != null)
							{
								for (Relation rel : relationStm.getRelations())
								{
									if (rel.getObject() != null)
									{
										for (String s : _personIds)
										{
											if (s.equals(rel.getObject().toString()))
											{
												return true;
											}
										}
	 ...
	 */
	public final void setPersonIds(final Vector<String> personIds)
	{
