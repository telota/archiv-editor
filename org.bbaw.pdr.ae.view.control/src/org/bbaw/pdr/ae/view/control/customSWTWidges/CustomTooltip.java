...
public abstract class CustomTooltip
{
			...
			if (event.widget instanceof Control)
			{
				...
				if (!event.widget.isDisposed())
				{
					...
					switch (event.type)
					{
						...
						// case SWT.MouseExit:
						/*
						 * Give some insets to ensure we get exit informations
						 * from a wider area ;-)
						 */
						...
						//
						// if
						// (!rect.contains(c.getDisplay().getCursorLocation()))
						// {
						// toolTipHide(shell, event);
						// }
						default:
	 ...
	 */
	public CustomTooltip(final Control control)
	{
	 ...
	 * @see #NO_RECREATE
	 */
	public CustomTooltip(final Control control, final int style, final boolean manualActivation)
	{
