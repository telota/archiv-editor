...
/**
 * The Class FilterOnlyWithoutLCCNPersonsHandler.
 * @author Christoph Plutte
 */
public class FilterOnlyWithoutLCCNPersonsHandler implements IHandler, IElementUpdater
{
	...
	public final Object execute(final ExecutionEvent event) throws ExecutionException
	{
		...
		Command command = cService.getCommand("org.bbaw.pdr.ae.view.main.commands.FilterOnlyWithoutLCCNPersons");
		State tState = command.getState(AEPluginIDs.TOGGLE_STATE_FILTERONLYWITHOUTLCCNPERSONS);
		_isChecked = (Boolean) tState.getValue();
		if (!_isChecked)
		{
			currentTree.addFilter(new PersonWithoutLCCNFilter());
		}
		else
		{
			...
			else
			{
				...
				{
					if (f instanceof PersonWithoutLCCNFilter)
					{
