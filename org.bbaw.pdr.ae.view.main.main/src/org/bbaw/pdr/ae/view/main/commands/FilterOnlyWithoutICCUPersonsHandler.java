...
/**
 * The Class FilterOnlyWithoutICCUPersonsHandler.
 * @author Christoph Plutte
 */
public class FilterOnlyWithoutICCUPersonsHandler implements IHandler, IElementUpdater
{
	...
	public final Object execute(final ExecutionEvent event) throws ExecutionException
	{
		...
		Command command = cService.getCommand("org.bbaw.pdr.ae.view.main.commands.FilterOnlyWithoutICCUPersons");
		State tState = command.getState(AEPluginIDs.TOGGLE_STATE_FILTERONLYWITHOUTICCUPERSONS);
		_isChecked = (Boolean) tState.getValue();
		if (!_isChecked)
		{
			currentTree.addFilter(new PersonWithoutICCUFilter());
		}
		else
		{
			...
			else
			{
				...
				{
					if (f instanceof PersonWithoutICCUFilter)
					{
