...
import org.bbaw.pdr.ae.view.main.preferences.provider.FavoriteMarkupFilter;
...
/**
 * The Class FavoriteMarkupPage.
 * @author Christoph Plutte
 */
public class FavoriteMarkupPage extends PreferencePage implements IWorkbenchPreferencePage
{
	...
	/** The add all markups. */
	private boolean _addAllMarkups = Platform.getPreferencesService().getBoolean(CommonActivator.PLUGIN_ID,
			"FAVORITE_MARKUP_ALL", false, null); //$NON-NLS-1$
	...
	/**
	 * Instantiates a new favorite markup page.
	 */
	public FavoriteMarkupPage()
	{
		super();
	...
	protected final Control createContents(final Composite parent)
	{
		...
		title.setText(NLMessages.getString("Preference_favorite_markup_title"));
		title.setLayoutData(new GridData());
		((GridData) title.getLayoutData()).horizontalSpan = 1;

		Button buttonAddAll = new Button(_mainComposite, SWT.CHECK);
		buttonAddAll.setSelection(_addAllMarkups);
		buttonAddAll.setLayoutData(new GridData());

		((GridData) buttonAddAll.getLayoutData()).horizontalSpan = 1;
		buttonAddAll.addSelectionListener(new SelectionAdapter()
		{
			@Override
			public void widgetSelected(final SelectionEvent se)
			{
				_addAllMarkups = !_addAllMarkups;
				_addToRight.setEnabled(!_addAllMarkups);
				_removeFromRight.setEnabled(!_addAllMarkups);
				_tree.setEnabled(!_addAllMarkups);
				_table.setEnabled(!_addAllMarkups);
			}
		}); // SelectionListener

		Label addAll = new Label(_mainComposite, SWT.None);
		addAll.setText(NLMessages.getString("Preference_add_all_markup"));
		addAll.setLayoutData(new GridData());
		((GridData) addAll.getLayoutData()).horizontalSpan = 1;
		...
		_treeViewer.addFilter(new FavoriteMarkupFilter(this));
		...
		_addToRight.setToolTipText(NLMessages.getString("Preference_add_to_favorite_markup_tooltip"));
		...
		_addToRight.addSelectionListener(new SelectionAdapter()
		{
			...
			public void widgetSelected(final SelectionEvent e)
			{
				...
				{
					_favoriteMarkups.put(cd.getValue(), cd);
					loadValues();
		...
		_removeFromRight.setToolTipText(NLMessages.getString("Preference_remove_from_favorite_markup_tooltip"));
		...
		_removeFromRight.addSelectionListener(new SelectionAdapter()
		{
			...
			public void widgetSelected(final SelectionEvent e)
			{
				...
				_addToRight.setEnabled(true);
				_favoriteMarkups.remove(cd.getValue());
				loadValues();
				Object[] elements = _treeViewer.getExpandedElements();
				_treeViewer.refresh();
				for (Object o : elements)
		...
		{
			_favoriteMarkups = new HashMap<String, ConfigData>();
		}
		loadValues();
		_addToRight.setEnabled(!_addAllMarkups);
		_removeFromRight.setEnabled(!_addAllMarkups);
	 ...
	 */
	public final HashMap<String, ConfigData> getFavoriteMarkups()
	{
	...
	final void loadValues()
	{
		...
		_markupTableViewer.setInput(_favoriteMarkups);
	...
	{
		CommonActivator.getDefault().getPreferenceStore().setValue("FAVORITE_MARKUP_ALL", _addAllMarkups); //$NON-NLS-1$
		boolean load = true;
		int i = 0;
		while (load)
		{
			String path = Platform.getPreferencesService().getString(CommonActivator.PLUGIN_ID,
					"FAVORITE_MARKUP" + i, null, null); //$NON-NLS-1$
			if (path != null && path.trim().length() > 0)
			{
				CommonActivator.getDefault().getPreferenceStore().setValue("FAVORITE_MARKUP" + i, "EMPTY"); //$NON-NLS-1$ //$NON-NLS-2$
				i++;
			}
			else
			{
		...
		List<ConfigData> configs = new ArrayList<ConfigData>(_favoriteMarkups.values());
		...
		for (ConfigData cd : configs)
		{
			...
			if (path != null)
			{
				CommonActivator.getDefault().getPreferenceStore().setValue("FAVORITE_MARKUP" + i, path); //$NON-NLS-1$
				i++;
			}
		}
		_facade.setFavoriteMarkupsToNull();
		return true;
	}
}
