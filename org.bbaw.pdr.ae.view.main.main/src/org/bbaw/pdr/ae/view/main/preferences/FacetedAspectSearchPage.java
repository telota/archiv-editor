...
import org.bbaw.pdr.ae.view.main.preferences.provider.FacetedAspectSearchFilter;
...
/**
 * The Class FacetedAspectSearchPage.
 * @author Christoph Plutte
 */
public class FacetedAspectSearchPage extends PreferencePage implements IWorkbenchPreferencePage
{
	...
	/**
	 * Instantiates a new faceted aspect search page.
	 */
	public FacetedAspectSearchPage()
	{
		super();
	...
	protected final Control createContents(final Composite parent)
	{
		...
		title.setText(NLMessages.getString("Preference_aspect_facet_proposal_title"));
		...
		_treeViewer.addFilter(new FacetedAspectSearchFilter(this));
		...
		_addToRight.addSelectionListener(new SelectionAdapter()
		{
			...
			public void widgetSelected(final SelectionEvent e)
			{
				...
				{
					_aspectFacetProposals.put(cd.getValue(), cd);
					loadValues();
		...
		_removeFromRight.addSelectionListener(new SelectionAdapter()
		{
			...
			public void widgetSelected(final SelectionEvent e)
			{
				...
				_addToRight.setEnabled(true);
				_aspectFacetProposals.remove(cd.getValue());
				loadValues();
				Object[] elements = _treeViewer.getExpandedElements();
				_treeViewer.refresh();
				for (Object o : elements)
		...
		_aspectFacetProposals = fpm.loadAspectFacetProposals();
		if (_aspectFacetProposals == null)
		{
			_aspectFacetProposals = new HashMap<String, ConfigData>();
		}
		loadValues();
		return null;
	}
	...
	final void loadValues()
	{
		...
		_markupTableViewer.setInput(_aspectFacetProposals);
	...
	public final boolean performOk()
	{
		...
		{
			String path = Platform.getPreferencesService().getString(CommonActivator.PLUGIN_ID,
					"ASPECT_FACET_PROPOSAL" + i, null, null); //$NON-NLS-1$
			if (path != null && path.trim().length() > 0)
			{
				CommonActivator.getDefault().getPreferenceStore().setValue("ASPECT_FACET_PROPOSAL" + i, "EMPTY"); //$NON-NLS-1$ //$NON-NLS-2$
				i++;
			}
			else
			{
		...
		List<ConfigData> configs = new ArrayList<ConfigData>(_aspectFacetProposals.values());
		...
		for (ConfigData cd : configs)
		{
			...
			if (path != null)
			{
				CommonActivator.getDefault().getPreferenceStore().setValue("ASPECT_FACET_PROPOSAL" + i, path); //$NON-NLS-1$
				i++;
			}
		}
		_facade.setAspectFacetProposals(_aspectFacetProposals);
		return true;
	}
}
