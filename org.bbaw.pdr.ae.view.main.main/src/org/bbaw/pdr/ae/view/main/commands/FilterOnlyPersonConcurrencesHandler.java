...
/**
 * The Class FilterOnlyPersonConcurrencesHandler.
 * @author Christoph Plutte
 */
public class FilterOnlyPersonConcurrencesHandler implements IHandler, IElementUpdater
{
	...
	public final Object execute(final ExecutionEvent event) throws ExecutionException
	{
		...
		Command command = cService.getCommand("org.bbaw.pdr.ae.view.main.commands.FilterOnlyPersonConcurrences");
		State tState = command.getState(AEPluginIDs.TOGGLE_STATE_FILTERONLYPERSONCONCURRENCES);
		_isChecked = (Boolean) tState.getValue();
		if (!_isChecked)
		{
			currentTree.addFilter(new PersonConcurrenceFilter());
		}
		else
		{
			...
			else
			{
				...
				{
					if (f instanceof PersonConcurrenceFilter)
					{
