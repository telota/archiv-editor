...
/**
 * The Class FilterOnlyPersonIdentifiersHandler.
 * @author Christoph Plutte
 */
public class FilterOnlyPersonIdentifiersHandler implements IHandler, IElementUpdater
{
	...
	public final Object execute(final ExecutionEvent event) throws ExecutionException
	{
		...
		Command command = cService.getCommand("org.bbaw.pdr.ae.view.main.commands.FilterOnlyPersonIdentifiers");
		State tState = command.getState(AEPluginIDs.TOGGLE_STATE_FILTERONLYPERSONIDENTIFIERS);
		_isChecked = (Boolean) tState.getValue();
		if (!_isChecked)
		{
			currentTree.addFilter(new PersonIdentifierFilter());
		}
		else
		{
			...
			else
			{
				...
				{
					if (f instanceof PersonIdentifierFilter)
					{
