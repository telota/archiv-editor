...
/**
 * The Class FilterOnlyWithoutVIAFPersonsHandler.
 * @author Christoph Plutte
 */
public class FilterOnlyWithoutVIAFPersonsHandler implements IHandler, IElementUpdater
{
	...
	public final Object execute(final ExecutionEvent event) throws ExecutionException
	{
		...
		Command command = cService.getCommand("org.bbaw.pdr.ae.view.main.commands.FilterOnlyWithoutVIAFPersons");
		State tState = command.getState(AEPluginIDs.TOGGLE_STATE_FILTERONLYWITHOUTVIAFPERSONS);
		_isChecked = (Boolean) tState.getValue();
		if (!_isChecked)
		{
			currentTree.addFilter(new PersonWithoutVIAFFilter());
		}
		else
		{
			...
			else
			{
				...
				{
					if (f instanceof PersonWithoutVIAFFilter)
					{
