...
/**
 * The Class FilterOnlyNewObjectsHandler.
 * @author Christoph Plutte
 */
public class FilterOnlyNewObjectsHandler implements IHandler, IElementUpdater
{
	...
	public final Object execute(final ExecutionEvent event) throws ExecutionException
	{
		...
		Command command = cService.getCommand("org.bbaw.pdr.ae.view.main.commands.FilterOnlyNewObjects");
		State tState = command.getState(AEPluginIDs.TOGGLE_STATE_FILTERONLYNEWOBJECTS);
		_isChecked = (Boolean) tState.getValue();
		if (!_isChecked)
		{
			currentTree.addFilter(new TreeNodeNewFilter());
		}
		else
		{
			...
			else
			{
				...
				{
					if (f instanceof TreeNodeNewFilter)
					{
